...
import { Upload, CheckCircle, AlertTriangle, FileSpreadsheet, ArrowLeft } from 'lucide-react';
        ...
        <div className="bg-slate-900 rounded-xl shadow-xl shadow-black/20 border border-slate-800 p-6 md:p-8">
            ...
            <div>
              <div className="bg-blue-950/40 border border-blue-900/50 p-4 rounded-lg mb-8 flex items-start gap-3">
                ...
                <p className="text-blue-200">
                  Arquivo <strong className="text-white">{arquivoInfo?.nome}</strong> lido com sucesso. Identificamos <strong className="text-white">{arquivoInfo?.linhas.length}</strong> linhas. <br/>
                  <span className="text-blue-300/80 text-sm">Por favor, associe as colunas da sua planilha com os campos exigidos pelo sistema abaixo.</span>
                </p>
              </div>
              ...
              <div className="flex flex-col-reverse md:flex-row justify-end gap-4 pt-4 border-t border-slate-800">
                ...
                <button onClick={() => setEtapa('PREVIA')} className="px-6 py-2.5 bg-blue-600 text-white font-medium rounded-lg hover:bg-blue-500 transition-colors shadow-lg shadow-blue-900/20">Ver Prévia dos Dados</button>
              </div>
            </div>
          )}
            ...
            <div>
              ...
              <div className="overflow-x-auto mb-8 border border-slate-800 rounded-lg bg-slate-950/50">
                <table className="w-full text-sm text-left text-slate-300">
                  <thead className="text-xs text-slate-400 uppercase bg-slate-900 border-b border-slate-800">
                    <tr>
                      {colunasBanco.map(c => <th key={c.key} className="px-4 py-3 font-medium">{c.label}</th>)}
                    </tr>
                  </thead>
                  <tbody>
              ...
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 pt-4 border-t border-slate-800">
                <div className="flex items-start md:items-center text-amber-500/90 gap-2 bg-amber-950/20 p-3 rounded-lg border border-amber-900/30">
                  ...
                  <span className="text-sm">Registros com dados duplicados serão ignorados automaticamente.</span>
                </div>
                <div className="flex gap-3 w-full md:w-auto">
                  ...
                  <button onClick={iniciarImportacao} className="flex-1 md:flex-none px-6 py-2.5 bg-emerald-600 text-white font-medium rounded-lg hover:bg-emerald-500 transition-colors shadow-lg shadow-emerald-900/20">Confirmar e Importar</button>
                </div>
              </div>
            </div>
          )}
          ...
          {etapa === 'PROCESSANDO' && (
            <div className="text-center py-16">
              ...
              <h2 className="text-xl font-semibold text-slate-200">Processando e validando <span className="text-blue-400">{arquivoInfo?.linhas.length}</span> registros...</h2>
              <p className="text-slate-400 mt-2">A inteligência artificial está organizando os dados. Isso pode levar alguns minutos.</p>
            </div>
          )}
          ...
          {etapa === 'SUCESSO' && (
            <div className="text-center py-12">
              ...
              <p className="text-slate-400 mb-8 max-w-lg mx-auto">
                Os dados foram processados com sucesso pelo motor analítico. O Histórico de Fornecedores, Clientes e Produtos foi atualizado no banco de dados.
              </p>
              <div className="flex justify-center gap-4">
                ...
                <button onClick={() => setEtapa('UPLOAD')} className="px-6 py-3 border border-slate-700 text-slate-300 font-medium rounded-lg hover:bg-slate-800 transition-colors">
